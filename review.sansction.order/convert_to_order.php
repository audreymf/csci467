...
// The sanctioned total is what was saved to subtotal during sanction.
// This is what we display and use as the base for any final discount.
$sanctionedTotal = (float)($quote['subtotal'] ?? $quote['total'] ?? 0);
...
<div class="info-box">
    ...
    <strong>Sanctioned Total</strong> (after sanction discount): $<?= number_format($sanctionedTotal, 2) ?>
</div>
...
<div class="form-box">
    ...
    <p style="font-size:13px; color:#555; margin-top:0;">
        Any discount here is applied <em>on top of</em> the sanction discount already reflected in the Sanctioned Total above.
    </p>
    <form method="POST">
        <label>Final Discount Type</label>
        <select name="final_discount_type" id="disc-type"
                onchange="document.getElementById('disc-val-row').style.display=this.value?'block':'none'">
        ...
        <p style="font-size:13px; color:#555;">
            Submitting will contact the external processing system, record the commission,
            and mark this quote as ordered.
        </p>
