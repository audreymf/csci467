...
/**
 * Compute totals using a known base (e.g. saved subtotal) instead of line items.
 * Used when a sanction discount was already saved so we don't double-dip.
 *
 * @return array{subtotal: float, total: float}
 */
function quote_compute_totals_from_base(float $base, ?string $discountType, $discountAmt): array
{
    $discountValue = (float)($discountAmt ?? 0);
    $total = $base;
    if ($discountType === 'percentage') {
        $total = $base - $base * ($discountValue / 100);
    } elseif ($discountType === 'amount') {
...
function quote_api_get_by_status(PDO $pdo, string $status): array
{
    ...
    foreach ($quotes as $q) {
        ...
        // If a sanction subtotal was saved, use it as the display total
        if ($q['subtotal'] !== null) {
            $totals = ['subtotal' => (float)$q['subtotal'], 'total' => (float)$q['subtotal']];
        } else {
...
function quote_api_get_one(PDO $pdo, int $id): array
{
    ...
    // Use saved subtotal (post-sanction discounted value) if available
    if ($quote['subtotal'] !== null) {
        $totals = ['subtotal' => (float)$quote['subtotal'], 'total' => (float)$quote['subtotal']];
    } else {
...
function quote_api_post_sanction(PDO $pdo, int $id, array $payload): array
{
    ...
    // Save the discounted total as subtotal so finalize can use it as its base
    $upd = $pdo->prepare(
...
function quote_api_post_order(PDO $pdo, int $id, array $payload): array
{
    ...
    // If a sanction discount was already baked into subtotal, use that as the base.
    // Then apply any additional final discount on top of it.
    $savedSubtotal = ($quote['subtotal'] !== null) ? (float)$quote['subtotal'] : null;

    if ($savedSubtotal !== null && $normalizedType !== null) {
        // Additional final discount applied on top of the sanctioned price
        $totals = quote_compute_totals_from_base($savedSubtotal, $normalizedType, (float)($finalDiscountAmt ?: 0));
    } elseif ($savedSubtotal !== null) {
        // No extra final discount — use saved sanctioned total as-is
        $totals = ['subtotal' => $savedSubtotal, 'total' => $savedSubtotal];
    } else {
        // No saved subtotal — fall back to computing from line items (legacy path)
        $items  = quote_get_line_items($pdo, $id);
    ...
    $upd->execute(['ordered', $effType, $effAmt, $commission, $id]);
